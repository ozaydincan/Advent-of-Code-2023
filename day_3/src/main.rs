...
impl SchematicDecoder {
    fn symbol_set(input: &str) -> HashSet<(i32, i32)> {
    ...
    fn part_1(input: &str, symbol_positions: &HashSet<(i32, i32)>) -> i32 {
            ...
            .flat_map(|(y, line)| {
                ...
                for (x, c) in line.chars().enumerate() {
                    if c.is_ascii_digit() {
                        ...
                        results.push((
                            y as i32,
                            x as i32 - current_num.len() as i32,
                            x as i32,
                            num_value,
                        ));
                        current_num.clear();
                ...
                if !current_num.is_empty() {
                    ...
                    results.push((
                        y as i32,
                        line.len() as i32 - current_num.len() as i32,
                        line.len() as i32,
                        num_value,
                    ));
                }
        ...
        input.lines().enumerate().for_each(|(y, line)| {
            ...
            line.chars().enumerate().for_each(|(x, c)| {
                if c.is_ascii_digit() {
                    ...
                    let x_end = x as i32;
                    for p in (y - 1..=y + 1)
                        .flat_map(|ch_y| (x_start - 1..x_end + 1).map(move |ch_x| (ch_y, ch_x)))
                    {
            ...
            if !current_num.is_empty() {
                ...
                for p in (y - 1..=y + 1)
                    .flat_map(|ch_y| (x_start - 1..x_end).map(move |ch_x| (ch_y, ch_x)))
                {
                    if let Some(vec) = star_map.get_mut(&p) {
                        vec.push(num_value);
                    }
