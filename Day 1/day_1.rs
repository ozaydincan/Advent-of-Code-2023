...
impl Decoder {
    fn part_1(infile:&String)->Result<i32, std::io::Error>{
        let sum = infile.lines().filter_map(|line| {
    ...
    fn part_2(infile:&String) -> Result<i32, std::io::Error>{
        let digit_map:HashMap<&str, usize> = [("0", 0), ("zero", 0),
        ("1", 1), ("one", 1),
        ("2", 2), ("two", 2),
        ("3", 3), ("three", 3),
        ("4", 4), ("four", 4),
        ...
        ("7", 7), ("seven", 7),
        ("8", 8), ("eight", 8),
        ("9", 9), ("nine", 9),].into_iter().collect();
        let sum: usize = infile
        .lines()
        .map(|line| {
            let (a, b) = digit_map
                .iter()
                .flat_map(|(key, &value)| line.match_indices(key).map(move |(idx, _)| (idx, value)))
                .minmax()
                .into_option()
                .unwrap();
            a.1 * 10 + b.1
